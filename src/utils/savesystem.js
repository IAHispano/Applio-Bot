...
const tagsMapping = {
	...
	Tags: {
		Artist: [
			...
			1185170437680922644, 1287069073926131792, 1288861820014170132,
		],
		Rapper: [
		...
		"TTS / Realtime": [
			...
			1287069337773015132, 1288862198244052992, 1288862251184295963,
		],
		RVC: [
...
function extractAlgorithm(text, removeHop = false) {
	...
	for (const pattern of regexPatterns) {
		const match = text.match(pattern);
		if (match) {
			let algorithm = match[0].replace(
				/^(.)(.*)$/,
				(m, first, rest) => first.toUpperCase() + rest.toLowerCase(),
			);

			if (removeHop) {
				const modifiedPattern = new RegExp(
					pattern.source + "\\s+" + hopNumbers + "(?![a-zA-Z0-9]|$)",
					"gi",
				);
				const hopMatch = text.match(modifiedPattern);
				if (hopMatch) {
					algorithm = algorithm.replace(
						new RegExp("\\b" + hopNumbers + "\\b"),
						"",
					);
...
function numberToReal(str) {
	...
	return matches.reduce(
		(updatedStr, num) => updatedStr.replace(num, num.replace(",", ".")),
		str,
	);
}
...
function eToEpochs(str) {
	...
	str = str.replace(
		pattern1,
		(_, num1, num2, num3) => ` ${num1 || num2 || num3} Epochs`,
	);
...
function extractEpochsAndAlgorithm(name, tags, content) {
	...
	let { result: modelType, newContent: modifiedName } = extractType(name, tags);
...
async function FormatThread(jsonData) {
	...
	if (links && links.length > 0) {
		links.forEach((link) => {
			const site = Object.keys(supportedSites).find(
				(site) =>
					link.includes(site) &&
					(site !== "cdn.discordapp.com" || link.includes(".zip")),
			);
