const {
  ...
  const { fetchModels } = require("../../../tools/civitai.js")
  module.exports = {
    data: new SlashCommandBuilder()
      ...
      .setDescriptionLocalizations({
        "es-ES":
          "civitai » Busca cualquier modelo de voz en civitai.",
      })
    ...
    async execute(interaction) {
      ...
      try {
        ...
        const response = await fetchModels({ limit: 100, page: page, query: model, sort: sort, period: period});
        ...
        async function displayPage(page) {
          ...
          for (let i = startIdx; i < endIdx; i++) {
            ...
            if (result.modelVersions[0].images[0].nsfw !== "X" &&
        ...
        menuCollector.on("collect", async (interaction) => {
          const selectedResult = data.find(
            (result) =>
              `${data.indexOf(result) + 1}-${result.id}-${result.modelVersions[0].createdAt}` ===
          ...
          if (selectedResult) {
            ...
            if (selectedResult.modelVersions[0].images[0].nsfw !== "X" &&
            result.modelVersions[0].images[0].nsfw !== "Mature") {
