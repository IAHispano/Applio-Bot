const { SlashCommandBuilder, EmbedBuilder, StringSelectMenuBuilder, ActionRowBuilder, SelectMenuBuilder, ComponentType } = require('discord.js');
...
module.exports = {
    ...
    const embed = new EmbedBuilder()
      ...
      .setColor('White') 
      .setDescription('**Navigate through our comprehensive guides to learn everything about Applio.**\n\n' + 
      ...
      .setFooter({
        text: `©️ Applio`, 
        iconURL: ('https://cdn.discordapp.com/avatars/1144714449563955302/92084831e7d14199a64a8b99bef95b73.webp?size=1024&format=webp&width=0&height=320') 
    });

    const beginnersGuide = new EmbedBuilder()
      .setTitle("Beginner's Guide") 
      .setColor('White') 
      .setDescription("🌱 **New to Applio? Start here!**\n\n" +
    ...
    const gettingStarted = new EmbedBuilder()
      ...
      .setColor('White') 
      .setDescription("🚀 **Ready to launch into Applio?**\n\n" +
    ...
    const guides = new EmbedBuilder()
      ...
      .setColor('White') 
      .setDescription("📖 **Dive deeper into Applio's functionalities.**\n\n" +
    ...
    const findVoiceModels = new EmbedBuilder()
      ...
      .setColor('White') 
      .setDescription("🎤 **Explore a vast library of voice models!**\n\n" + 
    ...
    collector.on('collect', (interaction) => {
      if (interaction.values[0] === "beginnersGuide") {
        interaction.update({ embeds: [beginnersGuide], components: components(false) }).catch((e) => { });
      } else if (interaction.values[0] === "gettingStarted") {
        interaction.update({ embeds: [gettingStarted], components: components(false) }).catch((e) => { });
      } else if (interaction.values[0] === "guides") {
        interaction.update({ embeds: [guides], components: components(false) }).catch((e) => { });
      } else if (interaction.values[0] === "findVoiceModels") {
        interaction.update({ embeds: [findVoiceModels], components: components(false) }).catch((e) => { });
      }
    });
