		...
		try {
			...
			if (sanitizedContent.length > 2000) {
				...
				if (profile) {
					...
					Voice.setLabel("🗣️ Made by Edge")
						.setStyle(ButtonStyle.Success)
						.setCustomId(`edge_tts`)
						.setDisabled(true);
					row = new ActionRowBuilder().addComponents(AI, User, Voice, Chat);
					const attachment = new AttachmentBuilder(audioFilePath).setName(
						"applio.mp4",
					);
					await interaction.followUp({
						files: [attachment],
				...
				collector.on("collect", async (i) => {
					if (i.customId === `chat_${interaction.user.id}`) {
						await interaction.followUp({
							content: `Now your next conversations will be converted to audio. <@${interaction.user.id}>`,
						});
						collector.stop();
						setUserPreference(interaction.user.id, "ChatToVoice", true);
					} else if (i.customId === `voice_${interaction.user.id}`) {
						await interaction.followUp({
							content: `Now your next conversations will be in text. <@${interaction.user.id}>`,
						});
						collector.stop();
