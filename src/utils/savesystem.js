...
const tagsMapping = {
  ...
  Tags: {
    ...
    Celebrity: [1175433585747034195, 1128777493789290507],
    ...
    RVC: [
      ...
      1128777435270361088, 1184575233223557213,
    ],
...
function reemplazarAbreviacionesEnTexto(texto) {
  ...
  const textoModificado = texto.replace(patron, (match) =>
    convertirAbreviacionANumero(match).toString(),
  );
...
function extractEpochsAndAlgorithm(cname, tags, content) {
  ...
  const regexPatterns = [
    /\((\d+)\s+Epochs\)/i,
    /\b(\d+)\s+Epochs\b/i,
    /-\s*(\d+)\s*Epochs?/i, // Format: " - {número} Epochs)"
    /\b(\d+)\s+Epochs\b/i,
    /\b(\d+)\s*Epochs\b/i,
    /\b(\d+)\s*Epoch\b/i,
    /(\d+) Epochs/i,
    / (\d+) Epochs/i,
    /\((\d+) Epochs\)/i, // Format: " {número} Epochs"
    /\(([^\)]*?(\d+)[^\)]*?)\s*Epochs\)/i,
    /(?:\s+\[|\()(\d+)\s+Epochs\)/i,
    /\[(\d+)\s*Epochs\]/i, // Format: "({texto}{número}Epochs)"
    /(\d+k)\s+Epochs/i,
    /Epochs\s*:\s*(\d+)/i,
    /Epoch\s*(\d+)/i,
    /(\d+)\s*(?:k\s*)?Epochs?/i,
    ...
    //wihout s
    / - (\d+)(?:\s+Epoch)?/i,
    / - (\d+)(?:\s+Epoch)?\)/i, // Format: " - {número} Epochs)"
    / (\d+) Epoch/i,
    /\((\d+) Epoch\)/i, // Format: " {número} Epochs"
    /\(([^\)]*?(\d+)[^\)]*?)\s*Epoch\)/i,
    /(?:\s+\[|\()(\d+)\s+Epoch\)/i,
    /\[(\d+)\s*Epoch\]/i, // Format: "({texto}{número}Epochs)"
    /(\d+k)\s+Epoch/i,
    //---
    /(\d+)\s*Epoch/i,
    /(\d+)\s+Epoch/i,
