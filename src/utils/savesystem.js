...
const tagsMapping = {
  Lang: {
    ES: [1108324682735820862, 1110874219362914425, 1127722904612778095, 1159339353198317628, 1175440470176960633, 1184575337691099268, 1233521366037172384, ], //1099149952652947456],
...
function ReplaceT(text) { //function ReplaceT(texto) {
  const patron = /\b(\d+(\.\d+)?[kmbn]{1,2})\b/ig;
  const modifiedText = text.replace(patron, (match) => ConvertN(match).toString());
...
async function JsonThread(thread, firstmessage, option, save=true) {
  ...
  let regexlink = /https?:\/\/(?!.*(?:youtu\.be|youtube|soundcloud|media\.discordapp\.net\/attachments)\b)(?![^\s]+\.(?:jpg|jpeg|png|gif|jpeg|bmp|svg|webp)\b)[^\s]+|(?:huggingface\.co|app\.kits\.ai|mega\.nz|drive\.google\.com|pixeldrain\.com)\/[^\s]+|[a-zA-Z0-9.-]+\/[\w.%-]+\.zip/g

  if (!firstmessage.attachments.size > 0 || !firstmessage.content.match(regexlink) || !thread.name.toLowerCase().includes("epoch")) {
    for (const message of messages.values()) {
      if(firstmessage.id === message.id) continue;
      const algorithm = extractAlgorithm(message.content);
      const isAihispanoOrTestAuthor = option === "aihispano" || message.author.id === firstmessage.author.id;
      const hasValidContent = message.content && message.content.match(regexlink);

      if (isAihispanoOrTestAuthor && (hasValidContent || message.attachments.size > 0 || message.content.includes("epoch") || !(algorithm === "N/A"))) {
          messageContent += " " + message.content + "\n";
          console.log(messageContent);
          firstmessage.attachments = firstmessage.attachments.concat(message.attachments);
          foundContent = true;
      }
    }
    if (foundContent) {
...
async function FormatThread(jsonData) {
  ...
  if (!reorganizedSupportedSites || reorganizedSupportedSites.length === 0) return {"Status": "Failed", "Reason": `Dont Found Links In ${jsonData.id}.json`};
  const seenLinks = {}; 
