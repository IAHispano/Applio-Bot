...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .setDescriptionLocalizations({
      "es-ES": "Fakeyou » Busca cualquier modelo de voz en Fakeyou.",
    })
  ...
  async execute(interaction) {
    ...
    try {
      ...
      async function displayPage(page) {
        ...
        const row_buttons = new ActionRowBuilder().addComponents(saveButton);
        mainEmbed = embed;
      ...
      menuCollector.on("collect", async (interaction) => {
        const selectedResult = data.find(
          (result) =>
            `${data.indexOf(result) + 1}-${result.model_token}-${
              result.created_at
